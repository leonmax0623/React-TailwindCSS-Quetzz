...
import { optional, requestStatusColor, requestStatuses } from '../../../helpers'
import clsx from 'clsx'

export default class RequestFilters extends Filters {
    componentDidMount() {
    ...
    filterList() {
        const filterClass = "mb-2 md:mr-16 md:mb-10 flex items-center p-4 md:p-0 bg-gray md:bg-white rounded-lg w-full"
        return [
            <span className={`${optional(this.state.filters, 'category.name') ? 'text-black' : 'text-gray-500'} ${filterClass}`}>
                ...
                <i
                    ...
                    onClick={() => this.updateFilter('category', null)}
                >
                </i>
            </span>,
            optional(this.state.filters, 'category.name') &&
                <span className={`${optional(this.state.filters, 'subcategory.name') ? 'text-black' : 'text-gray-500'} ${filterClass}`}>
                    <i className="text-gray fas fa-th-large mr-4"></i>
    ...
    render() {
        ...
        let filterWrapperClass = "md:w-auto flex items-center " + (this.props.doubleCol ? "w-1/2" : "w-full")
        if (this.isMobile()) {
        ...
        return <div className={this.props.className}>
            ...
            <div className="bg-white mb-6 pt-6 md:pt-10 pb-2 md:pb-0">
                <div className={`flex ${this.props.center ? 'justify-center' : 'justify-start'} flex-wrap pl-4 pr-2 md:px-10`}>
                {filterList.map(
                    (filter, filterIndex) => <div key={filterIndex} className={filterWrapperClass}>{filter}</div>
                )}
                </div>
            </div>
        </div>
    ...
    statusFilterIconColor(o) {
        const colorClass = requestStatusColor(optional(o, 'id'))
        if (colorClass !== "black") {
